DOCUMENT_TEMPLATE: str = """{document_name}文档里，目录层级为：{contents_title}，以下为正文内容：{context}"""

API_LIST = {
        "kimi":     {"url": "http://xx.xxx.xx.xx:8000/v1/chat/completions",
                     "headers": {"Content-Type": "application/json", "Authorization": "Bearer eyJh****************************nRlciIsImV4cCI6joxNzExOTU4NTE5LCJqdGkiOiJjbzU2anRxbG5sOTZlanZtcmlpMCIsInR5cCI6InJlZnJlc2giLCJzdWIiOiJjbzU2anRxbG5sOTZlanZtcmlnZyIsInNwYWNlX2lkIjoiY281Nmp0cWxubDk2ZWp2bXJpZzAiLCJhYnN0cmFjdF91c2VyX2lkIjoiY281Nmp0cWxubDk2ZWp2bXJpZmcifQ.M8DhrylzzWCL4KpYGk60TRq5ZpZtdrYHzrVjacDK4y37yD6oDj5JOmq6Zi3E16oUHT1BzIFW4O6BX4wdhcde_Q"}},
        "glm":      {"url": "http://xx.xxx.xx.xx:8001/v1/chat/completions",
                     "headers": {"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUz***************************.ibC7fFZrzzQtZgSlgqwD6lneb2QYp-Lstqe-WXTyP3w"}},
        "step":     {"url": "http://xx.xxx.xx.xx:8004/v1/chat/completions",
                     "headers": {"Content-Type": "application/json", "Authorization": "Bearer eyJhbGc***************************n***************************YmFuZWQiOmZhbHNlLCJleHAiOjE3MTE5NjA0NjYsIm1vZGUiOjIsIm9hc2lzX2lkIjo4NjAwNTczOTc2MzcyNDI4OCwidmVyc2lvbiI6MX0.vxMmxGP47AFCwdJEUaFZtHkB8rTqj7vj2qoTQ48XmaU...eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhcHBfaWQiOjEwMjAwLCJkZXZpY2VfaWQiOiJhYjRhMWE1MjcwYmZjOTBjNDRlMDJlNDhjYTljYzJmOTc3MzBlYjE4IiwiZXhwIjoxNzEzMjU0NjY2LCJvYXNpc19pZCI6ODYwMDU3Mzk3NjM3MjQyODgsInZlcnNpb24iOjF9.ooRjRVGsZcLWfcV9bxmVjKFE3DE9tSl-8IOcOeS48hM"}},
        "concise":  {"url": "http://xx.xxx.xx.xx:8006/v1/chat/completions",
                     "headers": {"Content-Type": "application/json", "Authorization": "Bearer 6618b407***************************632496f98087736c8de6e93"}},
        "qwen":     {"url": "http://xx.xxx.xx.xx:8003/v1/chat/completions",
                     "headers": {"Content-Type": "application/json", "Authorization": "Bearer geYguR***************************rgXUaIrrMUaL5WA5msOWQ0"}},
        "embedding":{"url": "http://127.0.0.1:8010/embedding",
                     "headers": {"Content-Type": "application/json"}},
        "reranker": {"url": "http://127.0.0.1:8010/reranker",
                     "headers": {"Content-Type": "application/json"}},
        "spark_embedding":{"url": "http(s)://emb-cn-huabei-1.xf-yun.com/",
                           "header": {"app_id": "2165b8f3", "status": 3}},
        "qwen1half-14b-chat":  {"url": "http://127.0.0.1:8020/v1/chat/completions",
                     "headers": {"Content-Type": "application/json"}}
    }


HYDE_TEMPLATE: str = """请编写一小段严谨、简洁的中文段落来回答以下问题，不得回答不知道、不得要求用户求助他人或拒绝回答，不要超过500字：
问题：{query}
段落："""


RAG_TEMPLATE: str = """你是一个知识丰富的学者，擅长根据搜索结果，回复用户的问题。你的性格也很温和，回复都很礼貌。

对于用户的每一个问题，你需要遵循下述回答流程：
流程一：如果搜索结果中有回答问题所需要的信息，基于这些搜索结果，尽量引用搜索结果中原文回复，信息要全面，避免任何主观臆断或虚构，且遵循以下回答要求：
1. 回复要全面，不漏信息，语言流畅，逻辑清晰。
2. 当问题比较模糊，而检索结果包含多种可能的答案时，反向提问用户想问的具体内容；
3. 可以对答案中的核心部分进行markdown加粗（**加粗内容**）。涉及到条目列举的内容需要在前面加序号并做分点描述，必要时可在每一点前面做小标题的汇总，并可以用总-分-总的形式展示分点式答案内容。
4. 答案尽量用礼貌用语开头，以“您好”开头，涉及到条目列举的内容需要在前面加序号并做分点描述，必要时可在每一点前面做小标题的汇总，并可以用总-分-总的形式展示分点式答案内容，使得答案内容可读性更强。
5. 使用引用标记来标注回答内容参考的搜索结果序号，例如[^1]（引用单个搜索结果）,[^1][^2]（引用多个搜索结果），其中方括号中的数字是搜索结果序号。引用标记只能出现在句尾标点符号前。


流程二：搜索结果中没有回答问题所需要的信息，遵循下述回答流程：
1. 直接回复“当前知识库找不到对应答案，我可以尝试用我的常识来回答你”。


【示例1】

搜索结果：
'''
[1] “（一）个人与其关联方之间的业务往来不符合独立交易原则而减少本人或者其关联方应纳税额，且无正当理由,
“（二）居民个人控制的，或者居民个人和居民企业共同控制的设立在实际税负明显偏低的国家（地区）的企业，无合理经营需要，对应当归属于居民个人的利润不作分配或者减少分配,
[2] 第六条已备案的中长期意向书在市场开展年度双边交易时，经双方协商后由任一方提交相应年度的双边交易意向书，电量调整幅度原则上不得超过15％，电价原则上保持不变。如不提交符合规定的年度双边交易意向书，电力用户、售电公司不得参与当年直接交易，按当年意向电量扣减发电企业市场电量上限，并将此失信行为纳入征信记录。
第七条省电力交易中心按照以下原则对所有年度双边交易意向电量进行确定。
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：如果商户月交易额度低于一定比例，会有什么后果？
思考：搜索结果中不包含回答问题所需信息，所以遵循流程二，我需要基于我的常识进行回答
回复：您好，当前知识库找不到对应答案，我可以尝试用我的常识来回答你。如果商户的月交易额度低于一定比例，可能会产生以下几种后果：
1. 手续费提高
2. 信誉度下降
3. 合作关系受影响
4. 资金流动性问题
总的来说，商户的月交易额度低于一定比例，可能会对商户的经营产生一定的负面影响。

【示例2】

搜索结果：
'''
[1] 澳门美食：鸡记
鸡记创立于2015年至今，是全澳门独家只卖鸡肉的粉面店。
美食推介：人参鸡汤米、胡椒猪肚鸡米、鲜茄酸辣米线、风沙鸡扒捞、手撕鸡捞面
[2] 澳门美食：甄其记面家
澳门美食：甄其记面家
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：鸡记有哪些美食推荐？
思考：搜索结果中包含回答问题所需信息，所以遵循流程一回答
回复：您好，鸡记的推荐美食有：人参鸡汤米、胡椒猪肚鸡米、鲜茄酸辣米线、风沙鸡扒捞、手撕鸡捞面[^1]。

【示例3】

搜索结果：
'''
{context}
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：{query}
思考：我要先选择回答流程，再请遵循格式和规则进行回答。内容要全面，要具体细化，但不要冗余，请一步一步思考。
回复："""

RAG_TEMPLATE_NEW: str = """你是一个知识丰富的学者，擅长根据搜索结果，回复用户的问题。你的性格也很温和，回复都很礼貌。

对于用户的每一个问题，基于这些搜索结果，尽量引用搜索结果中原文回复，信息要全面，避免任何主观臆断或虚构，且遵循以下回答要求：
1. 回复要全面，不漏信息，语言流畅，逻辑清晰。
2. 当问题比较模糊，而检索结果包含多种可能的答案时，反向提问用户想问的具体内容；
3. 可以对答案中的核心部分进行markdown加粗（**加粗内容**）；
4. 当提问需要进一步计算或者调用外部工具进行辅助时，可以调用外部工具进行辅助回答；
5. 使用引用标记来标注回答内容参考的搜索结果序号，例如[^1]（引用单个搜索结果），[^1][^2]（引用多个搜索结果），其中方括号中的数字是搜索结果序号。引用标记只能出现在句尾标点符号前。
6. 当要求你不加千位分隔符时，请去除数字中的逗号分隔符','。如：12,504,234.4->12504234.4, 1,233.23->1233.23, 234->234。
7. 当要求你保留小数点后2位有效数字时，请四舍五入保留小数点后2位小数。如：12.345->12.35, 33412.344->12.34, 12->12, 34.43->34.43。


【示例1】

搜索结果：
'''
[1] “（一）个人与其关联方之间的业务往来不符合独立交易原则而减少本人或者其关联方应纳税额，且无正当理由,
“（二）居民个人控制的，或者居民个人和居民企业共同控制的设立在实际税负明显偏低的国家（地区）的企业，无合理经营需要，对应当归属于居民个人的利润不作分配或者减少分配,
[2] 第六条已备案的中长期意向书在市场开展年度双边交易时，经双方协商后由任一方提交相应年度的双边交易意向书，电量调整幅度原则上不得超过15％，电价原则上保持不变。如不提交符合规定的年度双边交易意向书，电力用户、售电公司不得参与当年直接交易，按当年意向电量扣减发电企业市场电量上限，并将此失信行为纳入征信记录。
第七条省电力交易中心按照以下原则对所有年度双边交易意向电量进行确定。
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：如果商户月交易额度低于一定比例，会有什么后果？
思考：搜索结果中不包含回答问题所需信息，所以遵循流程二，我需要基于我的常识进行回答
回复：您好，当前知识库找不到对应答案，我可以尝试用我的常识来回答你。如果商户的月交易额度低于一定比例，可能会产生以下几种后果：
1. 手续费提高
2. 信誉度下降
3. 合作关系受影响
4. 资金流动性问题
总的来说，商户的月交易额度低于一定比例，可能会对商户的经营产生一定的负面影响。

【示例2】

搜索结果：
'''
[1] 中庚价值领航混合型证券投资基金2023年年度报告.txt文档里，目录层级为：无，以下为正文内容：318313581.12 3.73 医疗保健 594066240.89 6.97 工业 3862057.71 0.05 信息技术 11170647.70 0.13 通讯业务 262125236.06 3.07 房地产 730231505.09 8.56 合计 3766207623.44 44.16 注：以上分类采用全球行业分类标准（GICS）。 8.3 期末按公允价值占基金资产净值比例大小排序的所有股票投资明细 金额单位：人民币元 序号 股票代码 股票名称 数量(股) 公允价值(元) 占基金资 产净值比 例(%) 1 H01378 中国宏桥 142281500 823915998.54 9.66 2 H02186 绿叶制药 164536500 556166376.02 6.52 3 H09868 小鹏汽车－Ｗ 9146900 469992180.81 5.51 4 603283 赛腾股份 6436464 467029827.84 5.48 5 H00688 中国海外发展 30449500 379692695.45
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：请以json格式抽取2023年报告期末，中庚价值领航混合中占基金资产净值比例前5大的股票名称以及相应比例，键名为股票名称，键值为净值比例，净值比例以百分数表示，保留2位有效数字。
思考：搜索结果中具体的数据如下：股票名称1：中国宏桥，净值比例：9.66%；股票名称2：绿叶制药，净值比例：6.52%；股票名称3：小鹏汽车－Ｗ，净值比例：5.51%；股票名称4：赛腾股份，净值比例：5.48%；股票名称5：中国海外发展，净值比例：4.45%。需要以json格式输出，不能生成其它任何内容，不能生成引用。
回复：{{"中国宏桥": "9.66%", "绿叶制药": "6.52%", "小鹏汽车－Ｗ": "5.11%", "赛腾股份": "5.48%", "中国海外发展": "4.45%"}}

==========真实场景==========

搜索结果：
'''
{context}
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：{query}
思考：我要先选择回答流程，再请遵循格式和规则进行回答。内容要全面，要具体细化，但不要冗余，请一步一步思考。
回复："""


RAG_TEMPLATE_IE: str = """你是一个经验丰富的金融数据分析师，擅长根据搜索结果，抽取用户想要的信息。你的性格也很温和，回复都很礼貌。

对于用户的每一个问题，基于这些搜索结果，尽量引用搜索结果中原文回复，信息要全面，避免任何主观臆断或虚构，且遵循以下回答要求：
1. 回复要全面，不漏信息，语言流畅，逻辑清晰。
2. 当要求你不加千位分隔符时，请去除数字中的逗号分隔符','。如：12,504,234.4->12504234.4, 1,233.23->1233.23, 234->234。
3. 当要求你保留小数点后2位有效数字时，请四舍五入保留小数点后2位小数。如：12.345->12.35, 33412.344->12.34, 12->12, 34.43->34.43。
4. 当要求你以json格式输出时，请只输出json内容，不要生成其它任何内容，不要生成引用。如：{{"key1": "value1", "key2": "value2"}}。


【示例1】

搜索结果：
'''
[1] 中庚价值领航混合型证券投资基金2023年年度报告.txt文档里，目录层级为：无，以下为正文内容：318313581.12 3.73 医疗保健 594066240.89 6.97 工业 3862057.71 0.05 信息技术 11170647.70 0.13 通讯业务 262125236.06 3.07 房地产 730231505.09 8.56 合计 3766207623.44 44.16 注：以上分类采用全球行业分类标准（GICS）。 8.3 期末按公允价值占基金资产净值比例大小排序的所有股票投资明细 金额单位：人民币元 序号 股票代码 股票名称 数量(股) 公允价值(元) 占基金资 产净值比 例(%) 1 H01378 中国宏桥 142281500 823915998.54 9.66 2 H02186 绿叶制药 164536500 556166376.02 6.52 3 H09868 小鹏汽车－Ｗ 9146900 469992180.81 5.51 4 603283 赛腾股份 6436464 467029827.84 5.48 5 H00688 中国海外发展 30449500 379692695.45
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：请以json格式抽取2023年报告期末，中庚价值领航混合中占基金资产净值比例前5大的股票名称以及相应比例，键名为股票名称，键值为净值比例，净值比例以百分数表示，保留2位有效数字。
思考：搜索结果中具体的数据如下：股票名称1：中国宏桥，净值比例：9.66%；股票名称2：绿叶制药，净值比例：6.52%；股票名称3：小鹏汽车－Ｗ，净值比例：5.51%；股票名称4：赛腾股份，净值比例：5.48%；股票名称5：中国海外发展，净值比例：4.45%。需要以json格式输出，不能生成其它任何内容，不能生成引用。
回复：{{"中国宏桥": "9.66%", "绿叶制药": "6.52%", "小鹏汽车－Ｗ": "5.11%", "赛腾股份": "5.48%", "中国海外发展": "4.45%"}}

==========真实场景==========

搜索结果：
'''
{context}
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：{query}
思考：我要先选择回答流程，再请遵循格式和规则进行回答。内容要全面，要具体细化，但不要冗余，请一步一步思考。
回复："""


RAG_TEMPLATE_CHART: str = """你是一个经验丰富的金融数据分析师，擅长根据搜索结果，回复用户的问题，绘制统计图表。你的性格也很温和，回复都很礼貌。

对于用户的每一个问题，基于这些搜索结果，尽量引用搜索结果中原文回复，信息要全面，避免任何主观臆断或虚构，且遵循以下回答要求：
1. 回复要全面，不漏信息，语言流畅，逻辑清晰。不要回复你不会画图。
2. 当要求你不加千位分隔符时，请去除数字中的逗号分隔符','。如：12,504,234.4->12504234.4, 1,233.23->1233.23, 234->234。
3. 当要求你保留小数点后2位有效数字时，请四舍五入保留小数点后2位小数。如：12.345->12.35, 33412.344->12.34, 12->12, 34.43->34.43。


【示例1】

搜索结果：
'''
[1] “（一）个人与其关联方之间的业务往来不符合独立交易原则而减少本人或者其关联方应纳税额，且无正当理由,
“（二）居民个人控制的，或者居民个人和居民企业共同控制的设立在实际税负明显偏低的国家（地区）的企业，无合理经营需要，对应当归属于居民个人的利润不作分配或者减少分配,
[2] 第六条已备案的中长期意向书在市场开展年度双边交易时，经双方协商后由任一方提交相应年度的双边交易意向书，电量调整幅度原则上不得超过15％，电价原则上保持不变。如不提交符合规定的年度双边交易意向书，电力用户、售电公司不得参与当年直接交易，按当年意向电量扣减发电企业市场电量上限，并将此失信行为纳入征信记录。
第七条省电力交易中心按照以下原则对所有年度双边交易意向电量进行确定。
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：如果商户月交易额度低于一定比例，会有什么后果？
思考：搜索结果中不包含回答问题所需信息，所以遵循流程二，我需要基于我的常识进行回答
回复：您好，当前知识库找不到对应答案，我可以尝试用我的常识来回答你。如果商户的月交易额度低于一定比例，可能会产生以下几种后果：
1. 手续费提高
2. 信誉度下降
3. 合作关系受影响
4. 资金流动性问题
总的来说，商户的月交易额度低于一定比例，可能会对商户的经营产生一定的负面影响。


==========真实场景==========

搜索结果：
'''
{context}
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：{query}
思考：我要先选择回答流程，再请遵循格式和规则进行回答。内容要全面，要具体细化，但不要冗余，请一步一步思考。若要求去除千位分隔符，则需要去除数字中的逗号。
回复："""


GEN_QUERY = """你是一个问题生成器。输入一个文本，需要你根据文本内容生成若干[有意义的]问题。

要求：
- 你生成的问题应该[能够只根据输入文本进行回答]，你不应该生成与文本内容无关或者超出文本内容的问题。
- 你生成的问题应该[表达清晰、没有歧义]，易于理解。
- 你生成的问题应该[包含足够的上下文信息]，避免生成模糊的问题。
- 你生成的问题应该[涵盖输入文本中的不同方面]，而不仅仅是重复提问相同的内容。
- 你生成的问题不应该出现“文本中”之类的指代输入文本的表达。
- 你生成的问题数量不超过9个，输出时一行输出一个问题并用数字编号。
- 生成问题参考的文本分布要均匀，不要集中。

可生成的问题类型：
- 信息查询问题：关于输入文本中提到的描述的问题。例如：北京颁奖广场舞台的名字是什么？
- 原因与结果问题：探讨输入文本中的事件或现象的原因或结果的问题。例如：为什么人工智能在近年来变得如此流行？
- 比较与对比问题：关于比较输入文本中的两个或多个概念、观点或事物的异同的问题。例如：欧莱雅洗面奶和妮维雅洗面奶相比怎么样？
- 解释性问题：要求对输入文本中的概念或观点进行解释的问题。例如：解释一下抚州市建立健全生态产品价值实现机制是如何工作的？
- 意见与评价问题：针对输入文本中的观点进行询问或评价的问题。例如：治沙人对新发布的治沙政策的态度是积极的还是消极的？

【示例1】

文本：
收入结构持续改善，前三季度该行实现手续费及佣金净收入 94.65 亿元，同比增加 13.83 亿元，增幅 17.11%， 在营业收入中占比 24.39%，同比提升 2.12 个百分点。 此外，截至 9 月末，北京银行口径不良贷款余额 130.21 亿元，不良贷款率 1.25%， 贷款质量基本保持稳定。拨贷比 3.08%，拨备覆盖率 245.70%，拨备计提较为充足且保持较高水平。

请你根据上述要求生成[有意义的]问题：
1.前三季度该行实现手续费及佣金净收入多少？
2.前三季度营业收入中手续费及佣金净收入占比多少？
3.截至9月末，北京银行口径不良贷款余额是多少？
4.截至9月末，北京银行口径不良贷款率是多少？
5.截至9月末，北京银行拨贷比是多少？
6.截至9月末，北京银行拨备覆盖率是多少？
7.截至9月末，北京银行拨备计提水平如何？
8.前三季度该行手续费及佣金净收入同比增加多少？
9.前三季度该行手续费及佣金净收入增幅是多少？

【示例2】
文本：
加强对医务人员的关心爱护。全省各医疗机构实现医用防护物资3—7天库存，对医务人员院感防护进行培训和指导，援鄂医疗队保持零感染。采取分组轮班制、建制接管制等方式，合理安排好医务人员轮休及体检，一线医务人员工作强度明显降低。全力以赴救治感染医务人员，全省感染医务人员中92%已治愈出院。在落实国家规定补助待遇基础上，对一线医务人员（含援鄂医疗队员）发放一次性慰问补助，对援鄂医疗队员增发工作补助。在人事人才、子女上学、评选表彰等方面明确了倾斜政策。

请你根据上述要求生成[有意义的]问题：
1.为了爱护医务人员，采取了哪些措施？
2.援鄂医疗队员发放了什么补助？
3.对一线医务人员有哪些倾斜政策？
4.医疗机构实现了多久的医用防护物资库存？
5.对医务人员院感防护进行了什么培训和指导？
6.一线医务人员的工作强度有何变化？
7.援鄂医疗队的感染率是多少？
8.一次性慰问补助的金额是多少？
9.对医务人员有哪些评选表彰政策？

【示例3】
文本：
早在4年前，我已邀请从在美国完成MBA的专业人士为我们公司推动内部的沟通工作，使同事间与部门间在工作上的沟通更流畅和共识。自此以后三十多年来，公司在推动内部沟通的工作从未间断，希望将西方管理学的3C精神，既沟通、合作、协调融会贯通于整个工作环境，3C精神的推动，那是看起来容易的事，实际上推行却受到很多不同因素的影响，所以公司虽然推行3C精神有多年，现在仍然不断求去进步中。

请你根据上述要求生成[有意义的]问题：
1. 公司推动内部沟通的工作从何时开始？
2. 公司推行3C精神的目的是什么？
3. 公司推行3C精神受到什么影响？
4. 公司推行3C精神有多久了？
5. 为了推行3C精神，公司做了什么？
6. 为什么公司仍然在不断求去进步？
7. 公司推动内部沟通的工作有何成效？
8. 公司是如何将西方管理学的3C精神融会贯通于整个工作环境的？
9. 公司邀请了什么样的专业人士为公司推动内部沟通工作？

【示例4】
文本：
{context}

请你根据上述要求生成[有意义的]问题：
"""

GEN_QUERY_NEW = """你是一个问题生成器。输入一个文本，需要你根据文本内容生成若干[有意义的]问题。

要求：
- 你生成的问题应该[能够只根据输入文本进行回答]，你不应该生成与文本内容无关或者超出文本内容的问题。
- 你生成的问题应该[表达清晰、没有歧义]，易于理解。
- 你生成的问题应该[包含足够的上下文信息]，避免生成模糊的问题。
- 你生成的问题应该[涵盖输入文本中的不同方面]，而不仅仅是重复提问相同的内容。
- 你生成的问题不应该出现“文本中”之类的指代输入文本的表达，应该将代词替换为具体的实体。
- 你生成的问题应该清楚表达基金名称和报告时间信息，如“中庚价值领航混合”、“2022年”、“第二季度”、“年中报告”等。
- 你生成的问题数量不超过9个，输出时一行输出一个问题并用数字编号。
- 生成问题参考的文本分布要均匀，不要集中。

可生成的问题类型：
- 信息查询问题：关于输入文本中提到的描述的问题。例如：2023年，中庚价值领航混合在报告期末的基金份额总额为多少（以百分数表示，保留小数点后2位）？
- 计算与比较问题：实现数据之间的基本运算。例如：2022年，南方宝丰在报告期的份额净值增长率比2023年高多少（以百分数表示，保留小数点后2位）？
- 信息抽取格式化问题：用json格式抽取格式化信息，并规定格式。例如：请以json格式抽取2023年报告期末，人保利丰中占基金资产净值比例前5大的股票名称以及相应比例，键名为股票名称，键值为净值比例，净值比例以百分数表示，保留2位有效数字。
- 统计图绘制问题：针对给定的数据进行统计图的绘制，如饼状图、折线图等。例如：请根据2023年的四个季度报告，分析东方精选混合的股票投资占基金总资产的比例变化，并以季报时间为横轴、比例为纵轴绘制折线统计图。

【示例1】
文本：
《招商瑞丰灵活配置混合型发起式证券投资基金2023年第4季度报告.md》：5.11.5 报告期末前十名股票中存在流通受限情况的说明 §6 开放式基金份额变动 §7 基金管理人运用固有资金投资本基金情况 7.1 基金管理人持有本基金份额变动情况 7.2 基金管理人运用固有资金投资本基金交易明细 §8 报告期末发起式基金发起资金持有份额情况 §9 影响投资者决策的其他重要信息 9.1 报告期内单一投资者持有基金份额比例达到或超过20%的情况
| 1 | 113063 | 赛轮转债 | 08 | 01 | | 2 | 127063 | 贵轮转债 | 04 | 00 | 5 报告期末前十名股票中存在流通受限情况的说明 本基金本报告期末投资前十名股票中不存在流通受限情况。 §6 开放式基金份额变动 单位：份 | | | | | --| --| --| | 项目 | 招商瑞丰混合发起式A | 招商瑞丰混合发起式C | | 报告期期初基金份额总额 | 27 | 08 | | 报告期期间基金总申购份额 | 57 | 37 | | 减：报告期期间基金总赎回份额 | 84 | 28 | | 报告期期间基金拆分变动份额（份额减少以"-"填列） | | | | 报告期期末基金份额总额 | 00 | 17 | §7 基金管理人运用固有资金投资本基金情况 1 基金管理人持有本基金份额变动情况 本报告期内基金管理人无运用固有资金投资本基金的情况。 2 基金管理人运用固有资金投资本基金交易明细 本报告期内基金管理人无运用固有资金投资本基金的交易明细。 §8 报告期末发起式基金发起资金持有份额情况 该基金的发起份额承诺持有期限已满3年，发起份额已全部赎回。 §9 影响投资者决策的其他重要信息 1 报告期内单一投资者持有基金份额比例达到或超过20%的情况 | | | | | | | | | | --| --| --| --| --| --| --| --| | 投资者类别 | 报告期内持有基金份额变化情况序号 | 持有基金份额比例达到或者超过20%的时间区间 | 期初份额 | 申购份额 | 赎回份额 | 报告期末持有基金情况持有份额 | 份额占比 |

请你根据上述要求生成[有意义的]问题：
1.根据2023年第四季度报告，招商瑞丰混合发起式A在报告期内的基金总申购份额是多少？（保留小数点后2位，不加千位分隔符）
2.根据2023年第四季度报告，招商瑞丰混合发起式在报告期末的债券投资中，哪支债券的投资占基金资产净值的比例最高？
3.2023年第四季度报告期末，招商瑞丰混合发起式中，占基金资产净值比例前十大的股票与2023年第二季度报告期末的前十大股票相同的有几支？
4.请以json格式抽取2023年的第四季度报告中，开放式基金份额变动的情况，键名为项目名称，键值为份额比例，比例以百分数表示，保留小数点后2位。
5.请根据2023年第4季度报告，说明招商瑞丰混合发起式在报告期末，开放式基金份额变动的情况，并按报告期期初基金份额总额绘制折线统计图，以时间为横轴、份额总额为纵轴绘制折线统计图。

【示例2】
文本：
{context}

请你根据上述要求生成[有意义的]问题：
"""

SHEET_DESC = """你是一个理解表格的专家。请根据markdown表格的前10行，生成该表的描述信息。

要求：
1. 你需要总结表格的每一列，对列进行总结和理解，并列出相关信息。请不要生成要求之外的任何内容！
2. 要求列出列名，列名后给出示例，以及列值类型（字符串类型、数字值类型），最后给出列名的解释。列之间使用换行符'\\n'分隔。
假设有一个小学学校表格，包含学校名、所属地区、创办时间、类别、学生人数、教职工人数、教学班数量等列。列名后给出示例（例如清华附小是学校名的示例），以及列值类型（字符串类型、数字值类型），最后给出列名的解释。列之间使用换行符'\\n'分隔。

例如：
输入：| 0 | 1 | 2 | 3 | 4 | 5 | 6 |\\n|:---|:---|:---|:----|:---|:---|:---|\\n| 北京市小学学校汇总表 | nan | nan | nan | nan | nan | nan |\\n| 学校名 | 所属地区 | 创办时间 | 类别 | 学生人数 | 教职工人数 | 教学班数量 |\\n| 清华附小 | 西城区 | 1998 | 公立小学 | 2000 | 140 | 122 |\\n...
输出：表格列信息如下：\\n学校名：清华附小，字符串类型，代表小学学校的名称\\n所属地区：西城区，字符串类型，表示该小学学校所在的位置\\n创办时间：1998，数字值类型，表示该小学学校的创办时间\\n类别：公立小学，字符串类型，表示该小学学校所在的类别\\n学生人数：2000，数字值类型，表示该小学学校的学生数量\\n教职工人数：140，数字值类型，表示该小学学校的教职工数量\\n教学班数量：122，数字值类型，表示该小学学校的教学班数量

输入：{markdown}
输出："""

NL2PANDAS = """{desc}
提问：{query}"""

INTENTION_CLASSIFICATION = """你是一个专业的就业指导专家。请根据用户的问题，对其进行意图的理解和分类。
类别一共有四种：
- 1.【金融基础信息查询】，用户根据基金公告数据，针对基础信息的提问查询；
- 2.【金融核心指标统计计算】：用户根据基金公告数据，针对金融核心指标的简单计算和关联对比；
- 3.【结构化信息抽取】：用户根据基金公告数据，针对信息抽取需求，要求按json格式抽取结构化信息；
- 4.【基金分析报告和统计图生成】：用户根据基金公告数据，针对简单的基金分析需求，要求生成分析文本以及统计图。

请根据用户的提问分析其意图，并直接给出分类的类别序号1、2、3、4中的一个，请不要回复其它无关信息，只回复序号！

【示例1】
提问：根据2023年中期报告，东方新能源汽车主题混合的股票投资收益是多少？（保留2位有效数字，不加千位分隔符）
类别：1

【示例2】
提问：请以json格式抽取华泰柏瑞成长智选混合在2023年的基金经理及其证券从业年限，键名为基金经理名称，键值证券从业年限。
类别：3

【示例3】
提问：请根据2023年的四个季度报告，分析华泰柏瑞消费成长混合的基金份额净值变化情况，并以季报时间为横轴、份额净值为纵轴绘制折线统计图。
类别：4

【示例4】
提问：2023年的四个季度报告期内，财通裕惠63个月定开债券的超额收益率最高是多少？（以百分数表示，保留2位有效数字）
类别：2

【示例5】
提问：{query}
类别："""

IMAGE_GEN = """问：{query}

答：{desc}

根据以上信息，使用list和matplotlib生成图表绘制的python代码。请只生成代码块，不要生成其它内容，不要使用pandas。"""


DOCUMENT_SELECTED = """你是一个金融分析专家，善于理解用户的问题。你需要从用户的提问中，判断用户的意图。你有所有基金公司的报告资料，这些资料按时间段顺序记录（第一季度、第二季度、第三季度、中期报告、年度报告），你需要理解用户的提问涉及到哪一个资料或者哪几个资料，回复用户需要查询的资料时间段。
时间段的选项：【0】未提及是某个季度或中期或者年度，【1】只提及一个时间段（第某季度、中期或年度其中一个），【2】某两个时间段（第X季度和第Y季度或中期和年度），【3】提及了四个季度全部。请注意，“四个季度”以及“四季度”是包括第一到第四季度的四个时间，只有“第四季度”才是只有一个时间。

【示例1】
提问：人保利丰纯债在2023年报告期末的负债合计是多少？（保留小数点后2位，不加千位分隔符）
回答：1

【示例2】
提问：方正富邦天璇混合的基金经理是谁？
回答：0

【示例3】
提问：华泰柏瑞成长智选混合在2023年中期报告中的前十大重仓股和2023年年度报告中的前十大重仓股有多少只相同？
回答：2

【示例4】
提问：红塔红土盛世普益混合发起式在2023年四个季度报告期末基金份额净值的最大值是多少？（保留小数点后2位）
回答：3

【示例5】
提问：{query}
回答："""


RAG_TEMPLATE_XINGHUO = """

## 角色定位

你是一位经验丰富且极具洞察力的电子信息领域职业发展与就业策略高级顾问。你的核心任务是为电子信息工程、通信工程、微电子、集成电路、人工智能硬件、物联网工程、计算机科学与技术（偏硬件方向）等相关专业的学生（包括本科生、硕士生、博士生）以及初入职场的专业人士提供全面、精准、前瞻性的就业指导。

## 你的回答风格应：

专业严谨： 基于事实和数据，避免主观臆断。
条理清晰： 结构化输出，重点突出，易于理解。
深入浅出： 对于复杂概念能用通俗易懂的语言解释。
与时俱进： 结合最新的行业动态和市场需求。
积极中肯： 既能指出优势，也能客观分析不足，并给出改进方向。

## 互动要求：

当用户信息不足时，你会主动提出澄清性问题，以确保理解准确。
鼓励用户提供更详细的个人情况，以便你给出更精准的指导。
在回答时，如果涉及到具体公司或薪资，请强调其参考性和时效性。

## 请牢记以下搜索结果：
'''
{context}
'''
先判断搜索结果中是否包含回答问题所需内容，选择流程逻辑，然后进行回答，越详细越好，请回答以下问题：
提问：{query}
思考：我要先选择回答流程，再请遵循格式和规则进行回答。内容要全面，要具体细化，但不要冗余，请一步一步思考。
回复：
"""

HUNTER_TEMPLATE_XINGHUO = """
你是一位资深的AI招聘顾问。我将向你描述我正在招聘的职位详情，包括但不限于：职位名称、核心职责、所需的硬技能（如编程语言、工具软件）和软技能（如沟通能力、团队协作）、经验年限要求、薪资预算范围、公司文化特点以及我当前在招聘过程中遇到的具体挑战或困惑。

请你基于以上信息，为我提供一套全面的招聘优化建议方案。具体我希望你能包含以下几方面内容：

1.  **人才画像分析**：根据职位需求，精准描绘理想候选人的关键特征。
2.  **招聘渠道组合策略**：推荐最有效的招聘渠道（如特定招聘网站、社交平台、行业社群、内推等），并说明选择理由。
3.  **吸引候选人的亮点提炼**：帮助我挖掘并突出该职位和公司的独特吸引力。
4.  **面试流程与问题设计建议**：提供结构化面试流程建议，并针对核心能力设计一些有效的面试问题示例。
5.  **候选人评估与筛选技巧**：给出筛选简历和评估候选人的实用建议，以及如何识别潜力人才。
6.  **应对招聘挑战的策略**：针对我提出的具体困难，给出可行的解决方案。

请确保你的建议具有高度的针对性、创新性和可操作性，帮助我高效地找到合适的人才。
"""

TEST_INTENTION_CLASSIFICATION = """
# Role
你是一位深耕电子信息领域的资深就业战略顾问。你擅长从用户的模糊提问中精准捕捉其核心诉求（Intent），并提取关键实体（Entities），为后续的 RAG 检索和工具调用提供指导。

# Task
请分析用户的提问，将其归类到以下三个意图之一，并提取相关参数。

## 意图定义
1. **[KNOWLEDGE_GUIDANCE] (就业指导与资料查询)**
   - 场景：咨询简历撰写、面试技巧、职业规划、行业趋势、政策解读等知识性问题。
   - 关键词：怎么写、如何准备、流程、政策、前景、建议。
   
2. **[JOB_RETRIEVAL] (工作岗位精准获取)**
   - 场景：寻找具体的招聘信息、内推岗位、应届生补招或实习机会。
   - 关键词：校招、实习、招人吗、HC、投递、岗位需求。

3. **[COMPANY_INSIGHT] (公司背景与评价深度分析)**
   - 场景：咨询特定企业的口碑、薪资待遇、加班情况、技术氛围或面试面经。
   - 关键词：[公司名]怎么样、风评、薪资、加班吗、面试难吗。

# Output Format (Strictly JSON)
请仅返回以下 JSON 格式，不要包含任何解释文字：
{
  "reasoning": "简要分析用户为什么属于该意图",
  "intent": "KNOWLEDGE_GUIDANCE | JOB_RETRIEVAL | COMPANY_INSIGHT",
  "entities": {
    "company": "提取到的公司全称/简称，无则为null",
    "position": "提取到的岗位名称，无则为null",
    "location": "提取到的城市/地区，无则为null",
    "keywords": ["核心语义词1", "核心语义词2"]
  },
  "confidence": 0.0-1.0
}

# Few-shot Examples
- 用户：“双非电子信息本科，怎么改简历能进大厂？”
  输出：{"reasoning": "咨询简历修改建议，属于知识性指导", "intent": "KNOWLEDGE_GUIDANCE", "entities": {"company": null, "position": null, "location": null, "keywords": ["简历修改", "双非逆袭"]}, "confidence": 0.95}

- 用户：“现在上海字节跳动还有后端校招的HC吗？”
  输出：{"reasoning": "查询特定公司特定地区的实时岗位状态", "intent": "JOB_RETRIEVAL", "entities": {"company": "字节跳动", "position": "后端开发", "location": "上海", "keywords": ["HC", "校招"]}, "confidence": 0.98}

# Input Query
用户提问：{{USER_QUERY}}
"""

SCHEMA_TEMPLATE: str = """
You are an expert in knowledge graph schema extraction. Your task is to analyze the knowledge domain related to the text
and query provided by the user, and based on these content, construct a schema.

Please return the extracted schema in JSON format.

The JSON Schema structure should strictly follow to these rules:

- The root object should contain two keys: "edgelabels" and "vertexlabels".
- "edgelabels": An array, where each element represents an entity type.
  - Each entity type object should contain:
    - "name": (string) The name of the entity type.
    - "properties": (array) A list of properties name for this entity type.
- "vertexlabels": An array, where each element represents a relation type.
  - Each relation type object should contain:
  - "name": (string) The name of the relation type.
  - "source_label": (string) The name of the source edge label type connected by this relation.
  - "target_label": (string) The name of the target edge label type connected by this relation.
  - "properties": (array, optional) A list of properties for the relation itself (structure same as entity properties).


Refer to the following baseline example to understand the output generation requirements:
## Example:
### Input example:
#### text:
Meet Sarah, a 30-year-old attorney, and her roommate, James, whom she's shared a home with since 2010. James, 
in his professional life, works as a journalist.

#### graph schema example:
{"vertices":[{"vertex_label":"person","properties":["name","age","occupation"]}], "edges":[{"edge_label":"roommate", "source_vertex_label":"person","target_vertex_label":"person","properties":["date"]]}

## Text
Meet Sarah, a 30-year-old attorney, and her roommate, James, whom she's shared a home with since 2010.
James, in his professional life, works as a journalist. Additionally, Sarah is the proud owner of the website
www.sarahsplace.com, while James manages his own webpage, though the specific URL is not mentioned here.
These two individuals, Sarah and James, have not only forged a strong personal bond as roommates but have also
carved out their distinctive digital presence through their respective webpages, showcasing their varied interests
and experiences.

## Query
Q1: What is Sarah's occupation and age?
Q2: Who is James's roommate, and what does James do for a living?
Q3: What is the URL of the website owned by Sarah?
Q4: Since what year have Sarah and James been living together?
Q5: List all individuals mentioned and their known occupations.
"""
SCHEMA_TEXT_TEMPLATE: str ="""
You are an expert in knowledge graph schema extraction. Your task is to carefully read the knowledge graph description 
provided below, consider the domain and core attributes to which the description belongs, and based on these content, construct a schema.

Please return the extracted schema in JSON format.

The JSON Schema structure should strictly follow to these rules:

- The root object should contain two keys: "edgelabels" and "vertexlabels".
- "edgelabels": An array, where each element represents an entity type.
  - Each entity type object should contain:
    - "name": (string) The name of the entity type.
    - "properties": (array) A list of properties for this entity type.
      - Each property object should contain the name of the property.
- "vertexlabels": An array, where each element represents a relation type.
  - Each relation type object should contain:
  - "name": (string) The name of the relation type, try to use descriptive words.
  - "source_label": (string) The name of the source edge label type connected by this relation.
  - "target_label": (string) The name of the target edge label type connected by this relation.
  - "properties": (array, optional) A list of properties for the relation itself (structure same as entity properties).


Refer to the following baseline example to understand the output generation requirements:
## Example:
### Input example:
#### text:
Meet Sarah, a 30-year-old attorney, and her roommate, James, whom she's shared a home with since 2010. James, in his professional life, works as a journalist.

#### graph schema example:
{{"vertices":[{{"vertex_label":"person","properties":["name","age","occupation"]}}], "edges":[{{"edge_label":"roommate", "source_vertex_label":"person","target_vertex_label":"person","properties":["date"]]}}}}

## Text
{text}

## Query
{query}
"""
NEED_ENHANCEMENT_JUDGE_TEMPLATE = """
你是一个智能意图和耗时评级助手。你需要决定用哪个级别的检索策略来处理当前的查询速度和准确度。

【分级标准】
- 返回 0 (Level 0)：极度简单、单一的事实性问题或常见闲聊。比如：“什么是校招？”，“你好”，“深信服的总部在哪”。（直接匹配最好）
- 返回 1 (Level 1)：普通的单点查询，但可能句子很长、有很多无关词汇，或者是由于使用了指代词需要消除歧义。比如：“它和算法比起来怎么样”，“前端如果不会 Vue 怎么投递？”。
- 返回 2 (Level 2)：极其复杂、宽泛、或包含多重意图的专业问题，需要多步推理扩充知识面。比如：“没有好项目拿不出手该怎么包装简历？”，“微电子专业今年的秋招前景好吗，主要应该准备哪些技术栈？”

【输出要求】
请仅输出数字 0、1 或 2，不要任何解释。

用户提问：{query}
等级判定：
"""

SINGLE_REWRITE_TEMPLATE = """
你是一个专精于信息检索的打磨助手。你的任务是以极快的速度，将用户的话精简或补全为最利于向量检索的查询短句。

【要求】
1. 过滤掉语气助词和废话。
2. 提取最核心的专有实体。
3. 请不要对问题进行拆分和扩散，保持原意图。
4. 如果有模糊的代词，尽量用你理解的专业词汇补全。
5. 仅返回处理后的单句短语，不要有任何客套废话！

用户提问：{query}
最简查询词：
"""

MULTI_QUERY_REWRITE_TEMPLATE = """
你是一个资深的电子信息就业指导专家和搜索提示词专家。你的任务是将用户模糊或复杂的提问，拆解并改写成 3 个不同视角但语义相关的搜索查询词，以便从本地知识库中检索出最丰富的背景资料。

【改写策略】
- **视角分解**：将问题按行业趋势、具体方法、通用常识等方面拆解。
- **关键词提取**：丢弃无用词，保留核心专业实体（例如：硬件、简历、春招、算法、面试等）。
- **精简短语**：搜索词应尽量是精练的短语，而不是完整的长句子。

【输出格式（纯JSON序列）】
请仅返回如下JSON数组格式，不要包含任何多余字母、Markdown代码块（```json）或其他解释文字：
[
  "查询词1",
  "查询词2",
  "查询词3"
]

用户提问：{query}
"""

HYDE_TEMPLATE_XINGHUO = """
你是一个电子信息领域的资深面试官和职业导师。请基于以下用户的提问，**凭你的直觉和经验**，写一段严谨、专业的“完美回答草稿”（不超过 300 字）。

注意：
1. 这份回答将被用于“假设性文档嵌入 (HyDE)”技术去知识库中寻找相似维度的真实资料。
2. 因此，你的回答需要包含大量**相关领域的专业词汇、概念实体以及可能的指导策略**。
3. 不必考虑你说的每一句话是否百分百正确，只要语义连贯、方向正确、包含足够多的专业关键词即可。
4. 请不要输出多余的客套话。直接给出段落正文。

用户提问：{query}
假设性回答：
"""

PROFILE_EXTRACTION_TEMPLATE = """
# Role
你是一个用户画像信息提取引擎。你需要从最新一轮对话中，提取与用户个人背景相关的结构化信息，用于更新用户画像。

# 当前用户画像
```json
{current_profile}
```

# 最新对话
用户：{user_message}
助手：{assistant_message}

# 提取规则
1. 关注以下字段：
   - major（专业）、degree（学历：本科/硕士/博士）、graduation_year（毕业年份）
   - target_cities（意向城市列表）、tech_stack（技术栈列表）、job_preferences（岗位偏好列表）
   - offer_status（Offer状态，含 received/pending/rejected 三个子列表）
   - experience_level（经验水平：应届生/1-3年/3-5年/5年以上）、concerns（关注点/顾虑列表）
   - education_background（学历背景列表，每项含 school/level[985/211/双非]/degree/major）
   - project_experience（项目经历列表，每项含 name/tech_stack/description）
   - internship_experience（实习经历列表，每项含 company/position/duration）
2. 如果对话中**明确提到或暗示**了某个字段的信息，输出该字段的**增量值**
3. 对于简单列表字段（target_cities, tech_stack, job_preferences, concerns），只输出**新增**的条目
4. 对于复合对象列表（education_background, project_experience, internship_experience），输出完整的新条目对象
5. 对于 offer_status，输出具体变化的子字段
6. 如果本轮对话**完全不涉及**任何画像信息，请返回 `{"no_update": true}`
7. **不要凭空猜测**，只提取对话中实际出现的信息

# 输出格式（严格 JSON，不要 Markdown 代码块）
示例1：{"major": "电子信息工程", "target_cities": ["深圳"], "tech_stack": ["FPGA", "Verilog"]}
示例2：{"education_background": [{"school": "XX大学", "level": "211", "degree": "本科", "major": "电子信息"}]}
示例3：{"project_experience": [{"name": "智能车控制系统", "tech_stack": ["STM32", "PID"], "description": "基于STM32的智能循迹小车"}]}
示例4：{"internship_experience": [{"company": "华为", "position": "嵌入式开发实习生", "duration": "3个月"}]}
示例5：{"no_update": true}

请直接输出 JSON：
"""


# ============================================================
# LangGraph 反思机制相关 Prompt 模板
# ============================================================

DOC_GRADING_TEMPLATE = """
你是一个文档相关性评审员。给定用户问题和一段检索文档，判断该文档是否包含回答问题的有用信息。

用户问题：{query}
检索文档：{document}

请仅回复 "yes" 或 "no"。
"""

REFLECTION_TEMPLATE = """
你是一个严格的回答质量审核员。你需要判断以下回答是否忠实于给定的参考资料，不包含臆造或无中生有的内容。

## 用户问题
{query}

## 参考资料
{context}

## 模型回答
{generation}

## 评估标准
1. 回答中的每个事实论断是否都能在参考资料中找到依据？
2. 回答是否存在超出参考资料范围的主观推断？
3. 回答的结构和逻辑是否清晰？

请以 JSON 格式返回（不要 Markdown 代码块）：
{{"faithful_score": 0.0-1.0, "issues": ["问题描述1", "问题描述2"], "suggestion": "改进建议"}}
"""

REFLECTION_REWRITE_TEMPLATE = """
你是一个搜索查询优化专家。之前的检索结果未能很好地回答用户问题，请根据以下反思日志，生成一个改进后的查询词。

## 原始问题
{query}

## 反思日志
{reflection_log}

## 要求
1. 结合反思中指出的不足，调整搜索方向
2. 使用更精准的专业词汇
3. 仅返回改写后的查询短语，不要任何解释

改写后的查询：
"""

REWRITE_FIX_TEMPLATE = """
你是一个搜索查询修复专家。以下改写结果存在质量问题，请在保持原意的前提下给出一个更精准的改写词。

## 原始问题
{query}

## 有问题的改写词
{bad_rewrite}

## 存在的问题
{issue_description}

## 要求
1. 保留原问题的核心意图，不要引入原问题没有的实体
2. 去除冗余词汇，保留核心专业词汇
3. 长度控制在 5-30 个字
4. 仅返回改写后的短语，不要任何解释

修复后的查询词：
"""


# ============================================================
# 评测数据集生成专用 Prompt 模板
# ============================================================

EVAL_CHUNK_QA_TEMPLATE = """你是一个专业的 RAG 评测集出题专家。根据给定的文档片段，生成 2-3 个高质量的问答对。

### 出题要求
1. 问题必须可以**仅根据给定片段内容**完整回答，不得超出片段范围。
2. 答案应直接引用或忠实概括片段中的原文，禁止臆造信息。
3. 问题表述清晰，包含足够的上下文信息（如涉及的技术名称、概念、文档名等），避免使用"文中"、"上述"等模糊代词。
4. 问题类型应涵盖：事实查询、原因解释、操作步骤、概念对比等，保持多样性。
5. 问题难度为**简单到中等**，侧重信息的精确定位与提取。

### 文档来源
文件名：{file_name}

### 文档片段
{chunk_content}

### 输出格式（严格 JSON 数组，不要 Markdown 代码块，不要任何额外文字）
[
  {{"question": "问题1", "answer": "答案1"}},
  {{"question": "问题2", "answer": "答案2"}}
]
"""

EVAL_CROSS_CHAPTER_QA_TEMPLATE = """你是一个专业的 RAG 评测集出题专家。下面给出了一篇文档的完整摘要，请基于该摘要生成 2-3 个**需要跨章节综合理解**才能回答的高质量问答对。

### 出题要求
1. 问题必须需要综合文档中**多个章节或段落的信息**才能完整回答，不能是仅靠单段内容就能回答的简单题。
2. 答案应忠实概括文档摘要中的相关内容，禁止臆造。
3. 问题表述清晰，包含足够的上下文信息（如文档的主题、涉及的技术领域等），避免模糊代词。
4. 问题难度为**中等到较难**，考查跨章节的信息整合和逻辑推理能力。
5. 问题类型可包括：总结对比、流程串联、多条件综合判断、全局概念关联等。

### 文档来源
文件名：{file_name}

### 文档摘要
{summary}

### 输出格式（严格 JSON 数组，不要 Markdown 代码块，不要任何额外文字）
[
  {{"question": "问题1", "answer": "答案1"}},
  {{"question": "问题2", "answer": "答案2"}}
]
"""

EVAL_SUMMARY_TEMPLATE = """请用 300-500 字概括以下技术文档的核心内容，须涵盖文档涉及的主要章节结构及关键概念，保留专业术语。
仅输出摘要正文，不要有任何额外的标题、格式或前缀。

文档内容：
{content}
"""



# ============================================================
# 多路由意图分析 Prompt（意图识别 + 查询改写 合并为单次调用）
# ============================================================

MULTI_ROUTE_ANALYSIS_TEMPLATE = """
# Role
你是一位深耕电子信息领域的资深就业战略顾问，同时也是一名精通信息检索的查询优化专家。
你的任务是：**一步完成**用户意图的多标签分类，以及查询词的改写优化。

# 路由标签定义（可多选，DIRECT 与其余互斥）

| 标签 | 激活场景 |
|---|---|
| `RAG` | 知识性问题：简历撰写、面试技巧、职业规划、行业趋势、硬件技术栈、政策解读等 |
| `MCP_JOB` | 查询具体招聘岗位：HC 状态、投递方式、实习机会、内推、补招信息 |
| `MCP_COMPANY` | 查询特定公司信息：薪资水平、加班情况、口碑评价、面试难度、技术氛围 |
| `DIRECT` | 简单问候、闲聊、完全无需检索的通用回答（与以上标签互斥） |

# 查询改写规则
1. 过滤语气助词和冗余词汇，提取核心专业实体
2. 模糊代词（"它"、"这家公司"）用上下文中的具体词汇替换
3. 同时激活多路由时，改写后的 query 应兼顾所有激活路由的检索需求
4. 仅激活 DIRECT 时，rewritten_query 返回空字符串 ""

# Output Format (Strictly JSON, no markdown)
# 注意：
# 1. 所有键名与字符串值必须使用英文双引号。
# 2. 字段之间必须使用英文逗号分隔，禁止缺逗号、尾逗号、单引号或注释。
# 3. 只能输出一个 JSON 对象，禁止输出解释、前后缀文本或 Markdown 代码块。
{
  "reasoning": "简要分析用户意图及改写思路（1-2句）",
  "intents": ["RAG"],
  "rewritten_query": "优化后的检索短语",
  "entities": {
    "company": "公司名，无则null",
    "position": "岗位名，无则null",
    "location": "城市，无则null",
    "keywords": ["核心词1", "核心词2"]
  },
  "confidence": 0.0
}

# Few-shot Examples

**Example 1 — 单路由 RAG**
用户: "双非电子信息本科，怎么改简历能进大厂？"
输出: {"reasoning": "咨询简历修改建议，纯知识性指导，无需实时数据", "intents": ["RAG"], "rewritten_query": "双非本科电子信息简历优化 大厂求职技巧", "entities": {"company": null, "position": null, "location": null, "keywords": ["简历优化", "双非逆袭", "大厂"]}, "confidence": 0.95}

**Example 2 — 单路由 MCP_JOB**
用户: "现在上海字节跳动还有后端校招的HC吗？"
输出: {"reasoning": "查询特定公司实时岗位状态，需要外部实时招聘数据", "intents": ["MCP_JOB"], "rewritten_query": "字节跳动上海后端校招HC 2025", "entities": {"company": "字节跳动", "position": "后端开发", "location": "上海", "keywords": ["HC", "校招"]}, "confidence": 0.98}

**Example 3 — 多路由 RAG + MCP_COMPANY**  
用户: "华为的嵌入式岗位技术要求是什么，薪资怎么样？"
输出: {"reasoning": "既涉及技术知识（RAG可覆盖），又查询薪资实时评价（MCP_COMPANY）", "intents": ["RAG", "MCP_COMPANY"], "rewritten_query": "华为嵌入式岗位技术要求 薪资待遇", "entities": {"company": "华为", "position": "嵌入式开发", "location": null, "keywords": ["技术要求", "薪资", "嵌入式"]}, "confidence": 0.92}

**Example 4 — 三路由 RAG + MCP_JOB + MCP_COMPANY**
用户: "腾讯PCB设计岗的HC还有吗，技术要求和薪资待遇是啥？"
输出: {"reasoning": "查询HC状态、技术要求（知识库）和薪资（公司评价），三路同时激活", "intents": ["RAG", "MCP_JOB", "MCP_COMPANY"], "rewritten_query": "腾讯PCB设计工程师岗位 HC状态 技术栈 薪资", "entities": {"company": "腾讯", "position": "PCB设计", "location": null, "keywords": ["HC", "PCB", "薪资"]}, "confidence": 0.90}

**Example 5 — DIRECT 直接回答**
用户: "你好，你是谁？"
输出: {"reasoning": "简单问候，无需任何检索", "intents": ["DIRECT"], "rewritten_query": "", "entities": {"company": null, "position": null, "location": null, "keywords": []}, "confidence": 0.99}

# Input Query
用户提问：{{USER_QUERY}}
"""



# ============================================================
# 评测数据集 Answer 生成专用 Prompt 模板
# ============================================================

EVAL_ANSWER_GEN_TEMPLATE = """你是一个专业的技术问答助手。请严格根据给定的参考内容回答问题，不得引入参考内容之外的信息。

### 回答要求
1. 答案必须完全基于"参考内容"，禁止引入外部知识或主观推断。
2. 回答简洁准确，直接给出答案正文，不要重复问题，不需要客套语。
3. 如果参考内容不足以回答问题，请回复："参考内容中未找到相关信息。"
4. 专业术语保持原文不变，保持技术准确性。

### 参考内容
{chunk_content}

### 问题
{question}

### 回答
"""


if __name__ == "__main__":

    text = "context['chunks'][0]"
    query = "self._query"
    prompt_run = f"{SCHEMA_TEXT_TEMPLATE.format(text=text, query=query)}"
    print(prompt_run)
